...
const Hero = () => {
	...
	const isInViewHero = useInView(ref, { amount: 0.3, once: false });
	const setIsInViewHero = useStore((state) => state.setIsInViewHero);
	const setIsNavbarHidden = useStore((state) => state.setIsNavbarHidden);

	useEffect(() => {
		setIsInViewHero(isInViewHero);
		setIsNavbarHidden()
	}, [isInViewHero, setIsInViewHero, setIsNavbarHidden]);
	return (
		<div
			ref={ref}
